...
class MemberWorkspaceRepository {
    ...
    static async create(user_id, workspace_id, role = 'user') {
     console.log("CREAR MEMBER", { user_id, workspace_id, role })
        const query =
        ` INSERT INTO ${MEMBER_WORKSPACE_TABLE.NAME}(${MEMBER_WORKSPACE_TABLE.COLUMNS.FK_USER},${MEMBER_WORKSPACE_TABLE.COLUMNS.FK_WORKSPACE},${MEMBER_WORKSPACE_TABLE.COLUMNS.ROLE}) VALUES (?,?,?)`
...
static async getAllMembersByWorkspaceId(workspace_id) {
    const query = `
        SELECT 
            ...
            MW.${MEMBER_WORKSPACE_TABLE.COLUMNS.FK_USER} AS user_id,
            MW.${MEMBER_WORKSPACE_TABLE.COLUMNS.ROLE} AS member_role,
            U.${USERS_TABLE.COLUMNS.NAME} AS name,
            U.${USERS_TABLE.COLUMNS.EMAIL} AS email
        FROM ${MEMBER_WORKSPACE_TABLE.NAME} MW
        JOIN ${USERS_TABLE.NAME} U ON MW.${MEMBER_WORKSPACE_TABLE.COLUMNS.FK_USER} = U.${USERS_TABLE.COLUMNS.ID}
        WHERE MW.${MEMBER_WORKSPACE_TABLE.COLUMNS.FK_WORKSPACE} = ?
    `
